...
export default function Music() {
  ...
  onFrame(() => {
    ...
    // 原生后端读到真实时长后，把当前曲目的 durationMs 补一次，
    // 这样 UI 的进度/总时长是用真实值（避免 mock 的 0）。
    if (snap.durMs > 0) {
      const cur = track();
    ...
    const currentTrack = track();
    const duration =
      currentTrack.durationMs || snap.durMs || getTrackDuration(currentTrack);
...
function AlbumGrid(props: {
   ...
   * 传给 <Index each=...> 会强制重渲染卡片，保证封面实时刷新。 */
  ...
  /* 封面“指纹”：可见专辑的封面 key 拼接而成。
   * 封面 signal 一更新（懒加载完成），指纹变化 -> 滑块 key 变化 -> 强制重建，
   * 这样封面能立即刷出来，同时重建本身也会重放滑动动画。 */
  const coverStamp = createMemo(() =>
    props
      .albums()
      .slice(props.start(), props.start() + 3)
      .map((a) => props.covers()[a.id] ?? "_")
      .join("|"),
  );

  /* 每次光标/起始位置/封面变化都重建滑块：既回放滑动动画，又保证封面实时刷新。 */
  const sliderKey = () => `${props.start()}:${props.cursor()}:${coverStamp()}`;
