...
- **按 PS 回桌面，歌继续响。** 解码和出声都在云音自己的进程里，音频走系统的 BGM 口——MP3 / OGG / WAV / FLAC / OPUS 一视同仁，不会出现"某种格式退到桌面就哑了"。
- **撕页就停。** 不听了，在 LiveArea 上把云音撕掉就行：进程结束，声音立刻断，不用先进应用按暂停。
