...
> **一句话**：本地文件和在线流媒体都从同一条 `AudioSource → Decoder → BGM 输出` 的链路走，
> 上层不认识 HTTP，下层不认识网易云。
>
> **本文档只讲最终形态**（长什么样、边界在哪）。怎么改、先改什么见
> [重构计划.md](重构计划.md)；真机网络能力见 [网络探针实测.md](网络探针实测.md)。

本文描述「本地 + 在线」统一播放引擎的目标架构。它是重构任务书的落地版本，
并已按**当前实际代码**修正（任务书写于 M4A 支持之前，见文末差异一节）。

**适用范围**：现在只有本地播放运行；`source/ net/ provider/` 已有骨架但未接入运行时，
所以阅读时把"目标形态"和"当前实现"分开看 —— 每节都会标出当前状态。
...
| 模块 | 职责 | 现在 |
| --- | --- | --- |
| `rs/source/` | AudioSource 接缝、本地实现、HTTP Range 源、字节缓存 | **HTTP 源已真机验收（00.88 整曲播完）**；`cache.rs` 的字节缓存尚未接进来（现在用"双窗口预取"顶着） |
| `rs/net/` | 传输层（HTTP/TLS/Range/Cookie/取消）+ Phase 0 探针驱动 | 传输层与探针都已实机验证，生产路径（HttpRangeSource）也已跑通 |
| `rs/provider/` | Provider 接缝 + `AudioInfo` + 格式嗅探 | 已实现 |
| `rs/provider/netease/` | 网易云 API/加密/账号/URL 缓存 | 匿名 `outer/url` 已可用（`netease.ids`）；weapi/eapi 待 Phase 3 |
